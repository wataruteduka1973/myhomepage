  ...
  return <div className="about-page">
    ...
    <main id="about-main">
      ...
      <section className="about-process"><div className="section">
        ...
        <ol className="process-steps">{steps.map(([title, body], index) => <li key={title}><span className="process-icon"><StepIcon index={index} /></span><span className="step-number">0{index + 1}</span><h3>{title}</h3><p>{body}</p></li>)}</ol>
      ...
      <section className="section current-project" id="current"><p className="eyebrow">CURRENT PROJECT</p><span className="development-label">開発中</span><h2>市場分析ツール「システマ」</h2>
        ...
        <figure className="project-figure"><button className="project-preview" type="button" onClick={() => dialog.current?.showModal()} aria-haspopup="dialog" aria-label="システマの画面全体を拡大して見る"><img src="/systema-preview.png" alt="システマの開発画面。相場検索、価格分析、ターゲット分析などのメニューが並ぶ。" width="1920" height="1992" loading="lazy" /><span>画面全体を見る ↗</span></button><figcaption>開発中の画面です。公開時には内容やデザインが変わる場合があります。</figcaption></figure>
      </section>
